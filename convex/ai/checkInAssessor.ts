...
import type { ChatContentPart } from "../aiCoach/providers/index.js";
...
async function estimateBodyComposition(
  ...
  try {
    const imageContents: ChatContentPart[] = photoUrls.map((url) => ({
      type: "image_url" as const,
      imageUrl: url,
      detail: "low" as const,
    }));

    const raw = await getTextProvider().complete(
      [
        {
          role: "system",
      ...
      { maxTokens: 60, responseFormat: "json" },
    ) || "{}";
...
export const assessCheckIn = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    try {
      ...
      const assessment = await getTextProvider().complete(
        [
