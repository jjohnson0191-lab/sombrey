...
export const overview = query({
  args: { tzOffsetMinutes: v.number(), bodyBaseline: baselineValidator },
  handler: async (ctx, args) => {
    const user = await requireUser(ctx);
    const now = Date.now();
    const tz = args.tzOffsetMinutes;
    const data = await loadProgressData(ctx, user._id, now);
    const lr = loadRecovery(data.sessions, data.readiness, now, tz, 7);
    const today = dayKey(now, tz);
    return {
      strain: strainDay(data.sessions, now, tz),
...
export const performance = query({
  ...
  handler: async (ctx, args) => {
    ...
    return args.mode === "workouts"
      ? workoutPerformance(data.sessions, data.sets, args.subject, args.metric, args.range as RangeId | undefined, now, args.tzOffsetMinutes)
